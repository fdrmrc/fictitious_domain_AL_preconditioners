...
namespace Stokes
{
                            ...
                            Utilities::int_to_string(spacedim) + ">/")
    {
        ...
        parse_parameters_call_back.connect([&]() -> void
                                           { initialized = true; });
    }
    ...
    DistributedLagrangeProblem<dim, spacedim>::DistributedLagrangeProblem(
        ...
        : parameters(parameters),
          ...
          embedding_rhs_function("Embedding rhs function", spacedim),
          embedded_value_function("Embedded value", spacedim),
          embedding_dirichlet_boundary_function(
              "Embedding Dirichlet boundary conditions", spacedim + 1),
    ...
    void DistributedLagrangeProblem<dim, spacedim>::setup_grids_and_dofs()
    {
        ...
        if (space_grid->n_cells() < 2e6)
        { // do not dump grid when mesh is too fine
    ...
    void DistributedLagrangeProblem<dim, spacedim>::setup_embedding_dofs()
    {
        ...
        //     DoFTools::make_hanging_node_constraints(*space_dh, constraints);
        //     VectorTools::interpolate_boundary_values(*space_dh,
        //                                              0,
        //                                              Functions::ConstantFunction<spacedim>(0., spacedim),
        //                                              constraints,
        ...
        std::cout << "Number of degrees of freedom: " << space_dh->n_dofs() << " (" << n_u << '+' << n_p << ')' << std::endl;
        ...
        DoFTools::make_sparsity_pattern(
            *space_dh, coupling_table, dsp_stokes, constraints, false);
        ...
        rhs_stokes.reinit(dofs_per_block);
    ...
    void DistributedLagrangeProblem<dim, spacedim>::setup_embedded_dofs()
    {
        ...
        if (parameters.embedded_space_finite_element_degree > 0)
        {
            embedded_fe = std::make_unique<FESystem<dim, spacedim>>(
                FE_Q<dim, spacedim>(parameters.embedded_space_finite_element_degree) ^ spacedim);
        }
        else if (parameters.embedded_space_finite_element_degree == 0)
        {
            // otherwise, DG(0) elements for the multiplier
            embedded_fe = std::make_unique<FESystem<dim, spacedim>>(
                FE_DGQ<dim, spacedim>(parameters.embedded_space_finite_element_degree) ^ spacedim);
        }
        else
        {
    ...
    void DistributedLagrangeProblem<dim, spacedim>::setup_coupling()
    {
        ...
        NonMatching::create_coupling_sparsity_pattern(
            *space_grid_tools_cache, *space_dh, *embedded_dh, quad, dsp,
            AffineConstraints<double>(), ComponentMask({true, true, false}), ComponentMask(),
            *embedded_mapping);
    ...
    {
        {
            ...
            FEValues<spacedim> fe_values(*space_fe,
                                         quadrature_formula,
            ...
            for (const auto &cell : space_dh->active_cell_iterators())
            {
                ...
                embedding_rhs_function.vector_value_list(
                ...
                for (unsigned int q = 0; q < n_q_points; ++q)
                    for (unsigned int i = 0; i < dofs_per_cell; ++i)
                    {
                        for (unsigned int j = 0; j < dofs_per_cell; ++j)
                            local_matrix(i, j) += (fe_values[u1].gradient(i, q) *
                                                       fe_values[u1].gradient(j, q) +
                                                   fe_values[u2].gradient(i, q) *
                                                       fe_values[u2].gradient(j, q)) *
                                                  fe_values.JxW(q);
    ...
    template <int dim, int spacedim>
    void DistributedLagrangeProblem<dim, spacedim>::assemble_stokes()
    {
        stokes_matrix = 0;
        rhs_stokes = 0;
        ...
        for (const auto &cell : space_dh->active_cell_iterators())
        {
            ...
            cell->get_dof_indices(local_dof_indices);
            constraints.distribute_local_to_global(local_matrix, local_rhs, local_dof_indices, stokes_matrix, rhs_stokes);
        }
        ...
        std::cout << "nrows A: " << stokes_matrix.block(0, 0).m() << std::endl;
        std::cout << "nrows B: " << stokes_matrix.block(1, 0).m() << std::endl;
        std::cout << "nrows C: " << coupling_matrix.n() << std::endl;
        std::cout << "ncols C: " << coupling_matrix.m() << std::endl;
    }
    ...
    void DistributedLagrangeProblem<dim, spacedim>::solve()
    {
        ...
        else if (std::strcmp(parameters.solver.c_str(), "ELMAN_triang") == 0)
        {
            ...
            // K_inv_umfpack.initialize(stiffness_matrix);
            // auto K_inv = linear_operator(K, K_inv_umfpack);
            SolverControl control_cg(1000, 1e-2, false, false);
...
#ifdef DEAL_II_WITH_TRILINOS
            std::cout << "Solving with trilinos" << std::endl;
            ...
            SolverFGMRES<BlockVector<double>> solver_fgmres(schur_solver_control); //, data);

            solver_fgmres.solve(AA, solution_block, system_rhs_block, prec_elman);
...
// #endif
            ...
            SolverControl control_lagrangian(1000, 1e-2, false, true);
    ...
    void DistributedLagrangeProblem<dim, spacedim>::output_results()
    {
        ...
        {
            solution_names[i] = "vec_sol_" + std::string(1, 'x' + i); // "vec_sol_x", "vec_sol_y", ...
        }
        ...
        vecsol_out.add_data_vector(solution, solution_names, DataOut<spacedim>::type_dof_data, data_component_interpretation);
