...
public:
  ...
  unsigned int solve(const unsigned int ref_cycle = 0);
...
private:
  ...
  SparseMatrix<Number>
      stiffness_matrix_fg_plus_M; // A_2 + gamma*M, needed for modifiedAL
...
template <int dim> void EllipticInterfaceDLM<dim>::generate_grids() {
  ...
  if (parameters.do_convergence_study) {
    ...
    try {
      ...
      tria_fg.refine_global(parameters.initial_immersed_refinement);
      GridTools::rotate(numbers::PI / 6.0, tria_fg); // Rotate the immersed grid

      if constexpr (dim == 3)
        GridTools::rotate(Tensor<1, dim>({0.0, 1.0, 0.0}), numbers::PI / 3.0,
                          tria_fg); // Rotate the immersed grid
    } catch (const std::exception &exc) {
      std::cerr << exc.what() << std::endl;
...
void EllipticInterfaceDLM<dim>::assemble_subsystem(
  ...
  std::cout << "Assembling subsystem with mu = " << mu << std::endl;
...
template <int dim>
unsigned int EllipticInterfaceDLM<dim>::solve(const unsigned int ref_cycle) {
  ...
  std::cout << "h background = " << h_bg << std::endl;
  const double h_fg = GridTools::maximal_cell_diameter(tria_fg);
  std::cout << "h immersed = " << h_fg << std::endl;

  // W = h^2 M (!)

  // IDEAL ONE: same gammas
  // const double gamma = 10.;
  // parameters.gamma_AL_background = gamma / (h_fg * h_fg);
  // parameters.gamma_AL_immersed = gamma / (h_fg * h_fg);

  // MODIFIED: different gammas
  parameters.gamma_AL_background = 10 / (h_fg * h_fg);
  parameters.gamma_AL_immersed = 1e-2 / (h_fg * h_fg);
  // parameters.gamma_AL_background;

  assemble_subsystem(fe_fg, dof_handler_fg, constraints_fg,
                     stiffness_matrix_fg_plus_M, system_rhs_block.block(2),
...
#ifdef FALSE
  if (stiffness_matrix_bg.m() == 4225) {
    // Let's check for correctness
    BlockVector<double> ones(system_rhs_block);
    for (unsigned int b = 0; b < ones.n_blocks(); ++b)
      ones.block(b) = 1.;

    BlockVector<double> result(system_rhs_block);
    system_operator.vmult(result, ones);

    std::cout << "Result of system_operator * ones:" << std::endl;
    for (unsigned int b = 0; b < result.n_blocks(); ++b) {
      std::cout << "Block " << b << ":" << std::endl;
      for (unsigned int i = 0; i < result.block(b).size(); ++i)
        std::cout << result.block(b)[i] << std::endl;
    }

    std::cout << "Norm of the result: " << result.l2_norm() << std::endl;
  }
#endif
  ...
  build_AMG_augmented_block_scalar(
      dof_handler_bg, coupling_matrix, stiffness_matrix_bg, inverse_diag_mass,
      coupling_sparsity, constraints_bg, parameters.gamma_AL_background,
      parameters.beta_1, amg_prec_A11);

  // blocco aumentato = A_2 + gamma*h^{-2}*M*Minv*M = A_2 + gamma*h^{-2}*M
  ...
  amg_data.aggregation_threshold = 1e-1;
  ...
  amg_prec_A22.initialize(stiffness_matrix_fg_plus_M, amg_data);
  ...
  if (parameters.export_matrices_for_eig_analysis) {
    ...
    // export_to_matlab_csv(stiffness_matrix_bg,
    //                      parameters.output_directory + "/A_DLFDM_" +
    //                          std::to_string(ref_cycle) + ".csv");
    // export_to_matlab_csv(stiffness_matrix_fg,
    //                      parameters.output_directory + "/A_2_DLFDM_" +
    //                          std::to_string(ref_cycle) + ".csv");
    // export_to_matlab_csv(coupling_matrix,
    //                      parameters.output_directory + "/Ct_DLFDM_" +
    //                          std::to_string(ref_cycle) + ".csv");
    // export_to_matlab_csv(mass_matrix_fg,
    //                      parameters.output_directory + "/M_DLFDM_" +
    //                          std::to_string(ref_cycle) + ".csv");

    export_sparse_to_matlab_csv(stiffness_matrix_bg,
                                parameters.output_directory + "/A_DLFDM_" +
                                    std::to_string(ref_cycle) + ".csv");
    export_sparse_to_matlab_csv(stiffness_matrix_fg,
                                parameters.output_directory + "/A_2_DLFDM_" +
                                    std::to_string(ref_cycle) + ".csv");
    export_sparse_to_matlab_csv(coupling_matrix,
                                parameters.output_directory + "/Ct_DLFDM_" +
                                    std::to_string(ref_cycle) + ".csv");
    export_sparse_to_matlab_csv(mass_matrix_fg,
                                parameters.output_directory + "/M_DLFDM_" +
                                    std::to_string(ref_cycle) + ".csv");

    {
      DoFHandler<dim> dg_handler(tria_fg);
      FE_DGQ<dim> fe_dg(0);
      dg_handler.distribute_dofs(fe_dg);
      DynamicSparsityPattern dsp(dg_handler.n_dofs(), dof_handler_fg.n_dofs());
      // Since both spaces are on the same mesh, we iterate through
      // corresponding cells
      auto cell_q2 = dof_handler_fg.begin_active();
      auto cell_dgq1 = dg_handler.begin_active();

      for (; cell_q2 != dof_handler_fg.end(); ++cell_q2, ++cell_dgq1) {
        std::vector<types::global_dof_index> local_dof_indices_q2(
            fe_fg.n_dofs_per_cell());
        std::vector<types::global_dof_index> local_dof_indices_dgq1(
            fe_dg.n_dofs_per_cell());

        cell_q2->get_dof_indices(local_dof_indices_q2);
        cell_dgq1->get_dof_indices(local_dof_indices_dgq1);

        // Add coupling between all Q2 and DGQ1 DoFs on this cell
        for (unsigned int i = 0; i < fe_fg.n_dofs_per_cell(); ++i)
          for (unsigned int j = 0; j < fe_dg.n_dofs_per_cell(); ++j)
            dsp.add(local_dof_indices_dgq1[j], local_dof_indices_q2[i]);
      }

      SparsityPattern rectangular_sparsity;
      rectangular_sparsity.copy_from(dsp);
      SparseMatrix<Number> test_rectangular_mass_matrix;
      test_rectangular_mass_matrix.reinit(rectangular_sparsity);

      QGauss<dim> quadrature_formula(fe_fg.degree + fe_dg.degree + 1);

      FEValues<dim> fe_values_q2(fe_fg, quadrature_formula,
                                 update_values | update_JxW_values);

      FEValues<dim> fe_values_dgq1(fe_dg, quadrature_formula,
                                   update_values | update_JxW_values);

      const unsigned int dofs_per_cell_q2 = fe_fg.n_dofs_per_cell();
      const unsigned int dofs_per_cell_dgq1 = fe_dg.n_dofs_per_cell();
      const unsigned int n_q_points = quadrature_formula.size();

      FullMatrix<double> cell_matrix(dofs_per_cell_q2, dofs_per_cell_dgq1);
      ...
      // Iterate through corresponding cells in both DoF handlers
      cell_q2 = dof_handler_fg.begin_active();
      cell_dgq1 = dg_handler.begin_active();

      for (; cell_q2 != dof_handler_fg.end(); ++cell_q2, ++cell_dgq1) {
        fe_values_q2.reinit(cell_q2);
        fe_values_dgq1.reinit(cell_dgq1);

        cell_matrix = 0;

        // Assemble local matrix: (phi_i, psi_j) = integral of phi_i * psi_j
        for (unsigned int q_index = 0; q_index < n_q_points; ++q_index) {
          for (unsigned int i = 0; i < dofs_per_cell_q2; ++i) {
            for (unsigned int j = 0; j < dofs_per_cell_dgq1; ++j) {
              cell_matrix(i, j) += fe_values_q2.shape_value(i, q_index) *
                                   fe_values_dgq1.shape_value(j, q_index) *
                                   fe_values_q2.JxW(q_index);
            }
          }
        }
        ...
        for (unsigned int i = 0; i < dofs_per_cell_q2; ++i) {
          for (unsigned int j = 0; j < dofs_per_cell_dgq1; ++j) {
            test_rectangular_mass_matrix.add(local_dof_indices_dgq1[j],
                                             local_dof_indices_q2[i],
                                             cell_matrix(i, j));
          }
        }
      }

      std::cout << "   Rectangular matrix assembled successfully with size "
                << test_rectangular_mass_matrix.m() << " x "
                << test_rectangular_mass_matrix.n() << std::endl;

      export_sparse_to_matlab_csv(test_rectangular_mass_matrix,
                                  parameters.output_directory + "/C2_" +
                                      std::to_string(ref_cycle) + ".csv");

      SparsityPattern sparsity_W;
      SparseMatrix<Number> test_W_dg;
      AffineConstraints<double> dg_constraints;
      setup_stiffness_matrix(dg_handler, dg_constraints, sparsity_W, test_W_dg);
      //  mass matrix immersed
      Vector<double> dummy_vector(dg_handler.n_dofs());
      assemble_subsystem(fe_dg, dg_handler, dg_constraints, test_W_dg,
                         dummy_vector, 1., 0.,
                         Functions::ConstantFunction<dim>{0.});

      std::cout << "   test_W_dg matrix assembled successfully with size "
                << test_W_dg.m() << " x " << test_W_dg.n() << std::endl;

      export_sparse_to_matlab_csv(test_W_dg,
                                  parameters.output_directory + "/WDG_" +
                                      std::to_string(ref_cycle) + ".csv");
    }
  ...
  // setup-cost.
  {
    ...
    if (parameters.use_modified_AL_preconditioner) {
      ...
      if (parameters.gamma_AL_immersed > 20)
        std::cout << "WARNINIG: gamma_AL_immersed quite large. Just testing."
                  << std::endl;
